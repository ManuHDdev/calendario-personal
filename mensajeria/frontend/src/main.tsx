...
root.render(
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
    <div style={{ width: 24, height: 24, border: '2px solid #e5e5ea', borderTopColor: '#5ac8fa', borderRadius: '50%', animation: 'spin 700ms linear infinite' }} />
    <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
  </div>,
);

// mensajeria es de uso exclusivo del propietario: admin o mensajeria_admin
// (escritura) o mensajeria_invitado (solo lectura del inbox) — mismo patrón
// de guard que gastos/finanzas/frontend/src/main.tsx: login-required +
// comprobación de rol antes de montar la app.
keycloak
  ...
  .then((authenticated) => {
    ...
    const permitido = ['admin', 'mensajeria_admin', 'mensajeria_invitado'].some((r) => roles.includes(r));
    ...
    if (!permitido) {
      root.render(
        <div style={{
          ...
          <p style={{ fontSize: 14, color: '#8e8e93', margin: 0 }}>Necesitas el rol de administrador o invitado de Mensajería.</p>
          <button
