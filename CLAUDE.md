...
### Migraciones (OBLIGATORIO al tocar el esquema)
`init.sql` solo se ejecuta sobre un volumen vacío, así que **toda columna o
tabla añadida después del primer despliegue tiene que ir además en
`ofertas/infra/migrations/`**, o producción se queda atrás en silencio (una
base de datos desalineada da 500 en `/scraper/state` y `/searches/active`
mientras `/searches`, que es `SELECT *`, sigue en verde). Se aplican con
`bash ofertas/infra/migrate.sh`, son idempotentes y van en una transacción.
`backend/src/db/migrations.test.ts` compara init.sql con las migraciones y
...
- **`/health` estático en 10 de 11 backends**: `gastos`, `juegos`, `panel`, `paraisos`, `pisos`, `reparto`, `ruta`, `storage`, `watchlist` y `ytdl` devuelven `{status:'ok'}` sin comprobar nada. En los que tienen base de datos eso significa que el healthcheck de Docker sigue en verde mientras todas las rutas devuelven 500 — el contenedor parece sano justo cuando la app está inservible. `ofertas` ya lo comprueba de verdad (503 si Postgres no responde); el resto queda pendiente.
- **`pg.Pool` sin listener de `error`**: `gastos`, `mapacyd`, `paraisos`, `pisos`, `reparto`, `ruta` y `watchlist` crean el pool sin `pool.on('error', ...)`. En Node un evento `'error'` sin listener es una excepción no capturada, así que **cada reinicio de su Postgres mata el proceso del backend entero** (`57P01 terminating connection due to administrator command`). Reproducido y arreglado en `ofertas`; el resto queda pendiente.
