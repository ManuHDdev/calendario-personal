...
interface AppDef {
  ...
  nombre: string;
  color: string;
  roles: string[] | null; // null = visible para todos los roles autenticados
}
...
const APPS: AppDef[] = [
  ...
  { id: 'tenis',      nombre: 'Tenis',      color: '#d4f21e', roles: ['admin'] },
...
function getUrls(): Record<string, string> {
  ...
  return {
    ...
    tenis:      local ? 'http://localhost:5189/tenis/'    : '/tenis/',
  ...
  switch (id) {
    ...
    case 'tenis':
      return (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8">
          <circle cx="12" cy="12" r="9"/>
          <path d="M4.5 7.5c3 1.5 4.5 4 4.5 4.5s-1.5 3-4.5 4.5" strokeWidth="1.6"/>
          <path d="M19.5 7.5c-3 1.5-4.5 4-4.5 4.5s1.5 3 4.5 4.5" strokeWidth="1.6"/>
        </svg>
      );
    case 'finanzas':
      return (
...
interface Props {
  /** Si true el panel se abre hacia arriba (útil cuando el botón está al fondo de la pantalla) */
  upward?: boolean;
}
