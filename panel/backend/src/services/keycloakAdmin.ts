...
async function setUserRoles(userId: string, roleNames: string[]): Promise<void> {
  ...
  if (currentRoles.ok) {
    ...
    const appRoles = existing.filter((r) =>
      ['admin', 'familia', 'invitado', 'paraisos_admin', 'mapacyd_admin', 'mapacyd_invitado', 'reparto_admin', 'reparto_invitado'].includes(r.name),
    );
...
export async function listRoles(): Promise<string[]> {
  return ['admin', 'familia', 'invitado', 'paraisos_admin', 'mapacyd_admin', 'mapacyd_invitado', 'reparto_admin', 'reparto_invitado'];
}
