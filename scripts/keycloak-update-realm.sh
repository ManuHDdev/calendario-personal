...
# ── 4e. Asignar rol "mensajeria_admin" al usuario ───────────────────────────
MENSAJERIA_ADMIN_ROLE_JSON=$(curl -sf \
  "$HOST/admin/realms/$REALM/roles/mensajeria_admin" \
  -H "Authorization: Bearer $TOKEN")

MENSAJERIA_ADMIN_ROLE_ID=$(node -e "process.stdout.write(JSON.parse(process.argv[1]).id)" "$MENSAJERIA_ADMIN_ROLE_JSON")

HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  "$HOST/admin/realms/$REALM/users/$USER_ID/role-mappings/realm" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "[{\"id\":\"$MENSAJERIA_ADMIN_ROLE_ID\",\"name\":\"mensajeria_admin\"}]")

if [ "$HTTP_STATUS" = "204" ]; then
  ok "Rol 'mensajeria_admin' asignado a 'propietario'"
elif [ "$HTTP_STATUS" = "409" ]; then
  warn "Rol 'mensajeria_admin' ya estaba asignado (OK)"
else
  warn "Respuesta $HTTP_STATUS al asignar rol (puede que ya lo tenga)"
fi
...
REDIRECT_URIS='["http://localhost:4200/*","http://localhost:5173/*","http://localhost:5174/*","http://localhost:5175/*","http://localhost:5176/*","http://localhost:5177/*","http://localhost:5178/*","http://localhost:5179/*","http://localhost:5180/*","http://localhost:5181/*","http://localhost:5182/*","http://localhost:5183/*","http://localhost:5184/*","http://localhost:5185/*","http://localhost:5187/*","http://localhost:5188/*","https://elbunkerdelingeniero.duckdns.org/*"]'
