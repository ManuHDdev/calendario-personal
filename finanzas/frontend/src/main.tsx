...
root.render(
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
    <div style={{ width: 24, height: 24, border: '2px solid #e5e5ea', borderTopColor: '#eab308', borderRadius: '50%', animation: 'spin 700ms linear infinite' }} />
    <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
  </div>,
);

// finanzas es de acceso para admin e invitado (roles globales reutilizados) —
// mismo patrón que gastos/frontend/src/main.tsx: login-required + comprobación
// de rol antes de montar la app.
keycloak
  ...
  .then((authenticated) => {
    ...
    if (!roles.includes('admin') && !roles.includes('invitado')) {
      root.render(
        <div style={{
          ...
          <p style={{ fontSize: 14, color: '#8e8e93', margin: 0 }}>Necesitas el rol de administrador o invitado.</p>
          <button
