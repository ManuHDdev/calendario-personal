...
- [ ] 1.2 Tabla `normativa`: `comunidad` único, `distancia_farmacias_m` (default 250), `distancia_centros_sanitarios_m` nullable, `zona_excepcion` nullable, `notas`, `fuente_url`, soft delete, timestamps
- [ ] 1.3 Semilla de `normativa`: Madrid (250 / 150), Andalucía, Comunidad Valenciana, Canarias (+ fila de excepción 1.000 m para zonas turísticas de tipo común), Baleares — cada una con `fuente_url`
- [ ] 1.4 Tabla `farmacia`: `fuente`, `fuente_id`, `nombre`, `direccion`, `municipio`, `provincia`, `comunidad`, `latitud`, `longitud`, `precision_coordenadas`, `visto_en`, soft delete, timestamps, único `(fuente, fuente_id)`
- [ ] 1.5 Tabla `centro_sanitario`: misma forma + `tipo` (`primaria|especializada|hospital`)
- [ ] 1.6 Tabla `cobertura_municipio`: `municipio`, `provincia`, `farmacias_conocidas`, `poblacion`, `farmacias_esperadas`, `suficiente` (boolean), `calculado_en`
- [ ] 1.7 Tabla `busqueda`: `tipo` (`local|farmacia`), geografía (`comunidad`, `provincia`, `municipio`, `zona_texto`, `latitud`, `longitud`, `radio_km`), criterios de local (`precio_min/max`, `superficie_min/max`, `pie_calle`), criterios de farmacia (`facturacion_min/max`), viabilidad (`comprobar_farmacias`, `comprobar_centros_sanitarios`, `distancia_farmacias_m` nullable, `distancia_centros_sanitarios_m` nullable), `portales` text[], `ultimo_rastreo`, `ultimo_rastreo_error`, soft delete, timestamps
...
- [ ] 1.9 Tablas de apoyo: `geocode_cache` (patrón de `ruta`), `ruta_cache` (`origen_geo`, `destino_geo`, `motor`, `metros`), `scraper_state` (singleton), `presupuesto_rutas` (contador diario)
- [ ] 1.10 Índices: geográficos sobre `farmacia(latitud, longitud)` y `centro_sanitario(latitud, longitud)`, `anuncio(busqueda_id)`, `anuncio(veredicto)`, únicos de deduplicación
- [ ] 1.11 Trigger `update_updated_at_column()` reutilizado (patrón de `gastos`/`pisos`)

## 2. Backend scaffold (`locales/backend/`)

- [ ] 2.1 Scaffold Fastify + TypeScript copiando `pisos/backend` como punto de partida (`package.json`, `tsconfig.json`, `vitest.config.ts`)
- [ ] 2.2 `middleware/auth.ts` verbatim de `pisos/backend` (`verifyJwt`, `hasAnyRole`, `authMiddleware(['admin'])`)
- [ ] 2.3 `db/pool.ts` + parser de `NUMERIC` → number, igual que `pisos`
- [ ] 2.4 `GET /locales/api/health`
- [ ] 2.5 Env vars: `LOCALES_DB_*`, `KEYCLOAK_CERTS_URL`, `CORS_ORIGIN`, `PORT` (3013), `ORS_API_KEY`, `LOCALES_MOTOR_DISTANCIA`, `VALHALLA_URL`, `LOCALES_INTERVALO_MINUTOS`, `LOCALES_PAGINAS_POR_PORTAL`, `LOCALES_PRESUPUESTO_RUTAS_DIARIO`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_OWNER_CHAT_ID`, `NOMINATIM_USER_AGENT`

## 3. Padrón de farmacias y centros sanitarios

- [ ] 3.1 `padron/osm.ts` — importador Overpass (`amenity=pharmacy`; `amenity=clinic|doctors|hospital` + `healthcare=centre`), por comunidad, con reintentos y respeto de la cuota pública
- [ ] 3.2 `padron/oficial/madrid.ts` — importador del dataset `oficinas_farmacia` de `datos.comunidad.madrid` (CSV), geocodificando con Nominatim lo que venga sin coordenadas
- [ ] 3.3 `padron/fusion.ts` — deduplicación por proximidad (<40 m), la fuente oficial gana; ausencia en la fuente ⇒ soft delete, nunca borrado físico
- [ ] 3.4 `padron/cobertura.ts` — cota de cordura poblacional (~1 farmacia / 2.800 hab.) por municipio → `cobertura_municipio`
- [ ] 3.5 Refresco semanal desde el planificador + arranque en frío si el padrón está vacío
- [ ] 3.6 Tests: fusión de duplicados a distintas distancias, precedencia de la fuente oficial, soft delete al desaparecer, cálculo de cobertura

## 4. Módulo de viabilidad

- [ ] 4.1 `viabilidad/haversine.ts` — prefiltro de candidatas con `D * FACTOR_RODEO`, con el razonamiento (recta ≤ camino) en comentario y en test
- [ ] 4.2 `viabilidad/motor.ts` — interfaz `MotorDistancia` + selección por `LOCALES_MOTOR_DISTANCIA`
- [ ] 4.3 `viabilidad/motorOrs.ts` — `POST /v2/matrix/foot-walking`, troceado por el límite de destinos, 503 claro si falta `ORS_API_KEY`
- [ ] 4.4 `viabilidad/motorValhalla.ts` — `sources_to_targets` con `costing=pedestrian`
- [ ] 4.5 `viabilidad/presupuesto.ts` — contador diario; agotado ⇒ `sin_datos` con motivo, nunca descarte
- [ ] 4.6 `viabilidad/veredicto.ts` — semáforo con margen `E` por `precision_coordenadas`, degradación a ámbar por cobertura insuficiente, `NULL` de centros = comprobación no aplicable
- [ ] 4.7 Caché permanente en `ruta_cache` (coordenadas redondeadas a ~10 m)
- [ ] 4.8 Tests: los cuatro veredictos en sus fronteras exactas, degradación por cobertura, umbral nulo, presupuesto agotado, acierto y fallo de caché
...
- [ ] 7.6 `GET/PATCH /normativa` — consulta y edición de las distancias por comunidad
- [ ] 7.7 `POST /viabilidad/comprobar` — punto o dirección suelta → veredicto (lo que usa el bot y el botón de la UI)
- [ ] 7.8 `GET /padron/cobertura` — estado del padrón por municipio, para poder ver dónde no fiarse
- [ ] 7.9 Tests de rutas: guard de rol, Zod, soft delete, filtros

## 8. Bot de Telegram (doble sentido)

- [ ] 8.1 Telegraf con long polling en el proceso Fastify (patrón de `gastos`), ignorando en silencio otros chats
...
- [ ] 8.3 Recepción: ubicación compartida
- [ ] 8.4 Recepción: `/comprobar <dirección>` (precisión `exacta` si lleva número)
- [ ] 8.5 Recepción: URL de anuncio reenviada
...
- [ ] 10.1 `locales/backend/Dockerfile`, `locales/frontend/Dockerfile` + `nginx.conf`
- [ ] 10.2 `locales/infra/docker-compose.local.yml` y `.prod.yml` (red externa `calendario-net`), Postgres `locales` en `:5442`
- [ ] 10.3 Servicio opcional `locales-valhalla` + volumen de teselas, apagado por defecto
...
- [ ] 10.8 `locales/README.md`: cómo funciona, alta del bot, smoke, elección de motor, y el aviso legal
