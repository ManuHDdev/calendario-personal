...
// gastos/backend/src/middleware/auth.ts — misma duplicación aceptada y
// documentada en CLAUDE.md (no hay paquete compartido entre subapps, no se
// deduplica aquí). Única diferencia respecto al original: `verifyJwt` se
// exporta (además de quedar usado internamente por `authMiddleware`) porque
// `middleware/groupToken.ts` lo reutiliza para el guard combinado
// `authOrGroupToken` (design.md, "rutas y guards: tres niveles").
const CERTS_URL =
